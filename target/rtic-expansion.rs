#[doc = r" The RTIC application module"] pub mod app
{
    ...
    init(c : init :: Context) -> (Shared, Local, init :: Monotonics)
    {
        ...
        FunctionSpi > () ; let spi_master_sclk = pins.gpio26.into_mode :: <
        hal :: gpio :: FunctionSpi > () ; let spi_master_mosi =
        pins.gpio27.into_mode :: < hal :: gpio :: FunctionSpi > () ; let
        spi_master_miso = pins.gpio28.into_mode :: < hal :: gpio ::
        FunctionSpi > () ; let spi_master_cs = pins.gpio13.into_mode :: < hal
        :: gpio :: FunctionSpi > () ; let mut spi_dev = hal :: Spi :: < _, _,
        8 > :: new(p.SPI0) ; let mut spi_dev =
        ...
        ; let _mdio_pin = pins.gpio6.into_mode :: < hal :: gpio ::
        FunctionPio0 > () ; let _mdc_pin = pins.gpio7.into_mode :: < hal ::
        gpio :: FunctionPio0 > () ; let program = pio_proc :: pio_asm!
        ("
        .side_set 1", ".wrap_target", "set pins, 0   side 0",
        "start:", "pull block side 0", "set x, 31 side 0",
        "set pindirs, 1 side 0", "preamble:", "set pins, 1 side 1      [4]",
        ...
        = PIOBuilder ::
        from_program(installed).out_pins(6,
        1).side_set_pin_base(7).out_sticky(false).clock_divisor_fixed_point(SMI_DEFAULT_CLKDIV,
        PIO_CLK_DIV_FRAQ).out_shift_direction(ShiftDirection ::
        Right).in_shift_direction(ShiftDirection ::
        Left).autopush(true).autopull(false).set_pins(6,
        1).in_pin_base(6).build(sm0) ; sm.set_pindirs([(6, PinDir :: Output)])
        ; sm.set_pindirs([(7, PinDir :: Output)]) ; let smi_master =
    ...
    crate :: protocol :: slave :: Ready >)
    {
        ...
        } else if sr.host_config == ValidHostInterfaces :: Serial
        {
            (serial).lock(| serial |
            {
                ...
                [0 as u8, 20] ; write_serial(serial, "got here", false) ;
            }) ;
        }
    } struct Shared
    {
